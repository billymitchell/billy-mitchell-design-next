...
const MotionDesign = () => (
  <StaticQuery
    query={graphql`
    {
      allAirtable(filter: {table: {eq: "Project"}, data: {Creative_Discipline: {eq: "Motion Design"}, Published: {eq: true}, Featured: {ne: true}}}, sort: {order: DESC, fields: data___End_Date}) {
        nodes {
          recordId
          data {
            Project_Title
